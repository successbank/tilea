    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            <Label>태그</Label>
            <Input value={tags} onChange={(e) => setTags(e.target.value)} placeholder="쉼표로 구분 (예: CNC, 원목가구, 재단)" />
          </div>

          {error && <div className="rounded-lg bg-red-50 p-3 text-sm text-destructive">{error}</div>}

          <div className="flex gap-3">
            <Button onClick={handleSubmit} disabled={loading || !title || !content} className="flex-1">
              {loading ? '작성 중...' : '게시하기'}
            </Button>
