...
export async function PUT(request: Request, { params }: Ctx) {
  try {
    ...
    const existing = await prisma.tradePost.findFirst({ where: { id: params.id, userId: session.user.id } });
    if (!existing) return NextResponse.json({ error: '게시글을 찾을 수 없습니다' }, { status: 404 });
    const body = await request.json();
    const updated = await prisma.tradePost.update({ where: { id: params.id }, data: body });
...
export async function DELETE(_: Request, { params }: Ctx) {
  try {
    ...
    const existing = await prisma.tradePost.findFirst({ where: { id: params.id, userId: session.user.id } });
    if (!existing) return NextResponse.json({ error: '게시글을 찾을 수 없습니다' }, { status: 404 });
    await prisma.tradePost.delete({ where: { id: params.id } });
